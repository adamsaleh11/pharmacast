...
- Current Supabase project for frontend/backend auth is `https://ebrxagoygjtnpzlnxtmr.supabase.co`.
- Frontend env must use `NEXT_PUBLIC_SUPABASE_URL=https://ebrxagoygjtnpzlnxtmr.supabase.co`.
- Backend Supabase JWT issuer is `https://ebrxagoygjtnpzlnxtmr.supabase.co/auth/v1`.
- Backend JWKS URL is `https://ebrxagoygjtnpzlnxtmr.supabase.co/auth/v1/.well-known/jwks.json`.
- Valid current Supabase user access tokens for this project use `alg=ES256`, `kid=a1fd80b7-556b-4218-aa7f-76f060797299`, and `aud=authenticated`.
- Frontend must send `Authorization: Bearer <session.access_token>` to Spring Boot. Do not send the anon key, publishable key, refresh token, id token, stale HS256 token, or a token from another Supabase project.
- For backend auth responses, `401 AUTHENTICATION_REQUIRED` means missing/invalid/expired/wrong-project token. `403 USER_PROFILE_NOT_BOOTSTRAPPED` means the Supabase token is valid but the local app profile has not been created yet.
- `POST /auth/bootstrap` is allowed to run before a local app user row exists because it creates the user, organization, and first location. Do not reject bootstrap solely because the app profile is missing.
- Frontend has a Supabase auth callback at `/auth/callback` to exchange confirmation-link codes before returning to onboarding.
- After successful bootstrap, onboarding should navigate with a full page replacement to `/dashboard` so stale client auth context cannot redirect back to `/onboarding`.
- If auth debugging is needed, log only safe decoded JWT fields such as `alg`, `kid`, `aud`, `iss`, `sub`, `email`, `expiresAt`, and `now`; never log the raw token.
- If a browser shows an `HS256` token or wrong `iss`/`kid`, sign out, clear browser site data for the frontend origin, restart the Next.js dev server, and sign in again.
